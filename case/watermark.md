...
1、以`canvas`绘制水印，生成水印图片

2、以水印图片作为水印元素背景，将其添加到容器中

3、使用`MutationObserver` 监听元素删除、属性变化，重新生成水印

4、使用`ResizeObserver` 监听窗口变化，重新生成水印
...
type DefaultConfig = {
  ...
  /** 文本倾斜角度 */
  angle: number
  /** 每一处水印所占宽度（数值越大水印密度越低） */
  width: number
  /** 每一处水印所占高度（数值越大水印密度越低） */
  height: number
}
